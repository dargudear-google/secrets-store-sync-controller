...
func (r *SecretSyncReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, err error) {
	...
	defer func() {
		r.MetricReporter.ReportSyncK8SecretCtMetric(ctx, providerName)
		if err == nil {
			r.MetricReporter.ReportSyncK8SecretDuration(ctx, time.Since(startTime).Seconds())
		} else {
			r.MetricReporter.ReportSyncK8SecretErrCtMetric(ctx, "gcp", errorType)
		}
	}()
