...
@test "Validate metrics" {
  ...
  for pod_ip in $(kubectl get pod -n kube-system -l app=secrets-store-sync-controller -o jsonpath="{.items[0].status.podIP}")
  do
    run kubectl exec "${curl_pod_name}" -n metrics -- curl http://"${pod_ip}":8095/metrics
    assert_match "secret_sync_reconciliation_total" "${output}"
    assert_match "secret_sync_reconciliation_error_total" "${output}"
  done
